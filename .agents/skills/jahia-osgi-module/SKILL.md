...
- [`../../../CLAUDE.md`](../../../CLAUDE.md) — project identity and conventions.
- The `pom.xml` at the repo root — confirms parent version and packaging.
- `src/main/resources/META-INF/definitions.cnd` — node type definitions.
- `src/main/import/repository.xml` — initial JCR import.
...
### Alternative — `ManagedService` (legacy, avoid for new code)

The older pattern registers the component as both the service and a `ManagedService`, then implements `updated(Dictionary)`. It works but is verbose and couples your component to a lower-level OSGi API. Prefer the DS pattern above for new modules.
