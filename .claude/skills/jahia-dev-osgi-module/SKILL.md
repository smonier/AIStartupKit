...
> ⚠️ **The first line must be `# default configuration - won't be overriden` (exact string).** Jahia reads this marker and does NOT overwrite the file in `digital-factory-data/karaf/etc/` when the module is redeployed. Without it, every redeploy resets any operator-edited values (API keys, URLs) back to the shipped defaults — losing all environment-specific configuration.
